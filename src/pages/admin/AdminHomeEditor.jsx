...
function AdminHomeEditor() {
  console.log('🧠 AdminHomeEditor loaded');
  ...
  useEffect(() => {
    const fetchContent = async () => {
      try {
        setIsLoading(true);
        setError(null);
        console.log('AdminHomeEditor: Fetching content from Supabase...');

        const { data: { session }, error: authError } = await supabase.auth.getSession();
        console.log('🔐 AdminHomeEditor session:', session);
        if (authError || !session?.user) {
          console.error('AdminHomeEditor: Authentication error:', authError?.message || 'No user found');
          setError('User not authenticated. Please log in to access this page.');
          navigate('/login', { replace: true });
          return;
        }
        console.log('AdminHomeEditor: Authenticated user:', session.user.id, 'Role:', session.user.role);

        const { data: records, error: fetchError } = await supabase
          .from('home_content')
          .select('*');
        if (fetchError) {
          console.error('AdminHomeEditor: Fetch error:', fetchError);
          throw new Error(`Failed to fetch content: ${fetchError.message} (Code: ${fetchError.code || 'Unknown'})`);
        }
        console.log('AdminHomeEditor: Fetched records:', records);
        ...
        if (records && records.length > 0) {
          ...
          console.log('AdminHomeEditor: No records found, initializing default data');
        ...
        console.error('AdminHomeEditor: Error fetching content:', err);
    ...
    const initializeDefaultData = async () => {
      try {
        const { data: { session }, error: authError } = await supabase.auth.getSession();
        if (authError || !session?.user) {
          console.error('AdminHomeEditor: Authentication error during initialization:', authError?.message);
          return;
        }

        console.log('AdminHomeEditor: Initializing default data in Supabase');
        ...
        console.log('AdminHomeEditor: Default data initialized');
      } catch (err) {
        console.error('AdminHomeEditor: Error initializing default data:', err);
      }
    };
  ...
  const handleSave = async () => {
    ...
    setError(null);
    try {
      console.log('AdminHomeEditor: Saving content to Supabase...');
      const { data: { session }, error: authError } = await supabase.auth.getSession();
      if (authError || !session?.user) {
        console.error('AdminHomeEditor: Authentication error:', authError?.message);
        throw new Error('User not authenticated. Please log in to save changes.');
      }
      console.log('AdminHomeEditor: Saving as authenticated user:', session.user.id, 'Role:', session.user.role);
      ...
      if (heroFetchError && heroFetchError.code !== 'PGRST116') {
        console.error('AdminHomeEditor: Hero fetch error:', heroFetchError);
      ...
      if (heroRecord) {
        const { error: heroUpdateError } = await supabase
          ...
          .eq('section', 'hero');
        if (heroUpdateError) {
          console.error('AdminHomeEditor: Hero update error:', heroUpdateError);
        ...
        if (heroInsertError) {
          console.error('AdminHomeEditor: Hero insert error:', heroInsertError);
      ...
      if (tipFetchError && tipFetchError.code !== 'PGRST116') {
        console.error('AdminHomeEditor: Tip fetch error:', tipFetchError);
      ...
      if (tipRecord) {
        const { error: tipUpdateError } = await supabase
          ...
          .eq('section', 'tip_of_the_week');
        if (tipUpdateError) {
          console.error('AdminHomeEditor: Tip update error:', tipUpdateError);
        ...
        if (tipInsertError) {
          console.error('AdminHomeEditor: Tip insert error:', tipInsertError);
      ...
      if (fetchFeaturesError) {
        console.error('AdminHomeEditor: Features fetch error:', fetchFeaturesError);
      ...
      if (existingFeatures.length > 0) {
        ...
        if (deleteFeaturesError) {
          console.error('AdminHomeEditor: Features delete error:', deleteFeaturesError);
      ...
      for (const feature of data.keyFeatures) {
        ...
        if (featureInsertError) {
          console.error('AdminHomeEditor: Feature insert error:', featureInsertError);
          throw new Error(`Failed to insert feature: ${featureInsertError.message}`);
        }
      }

      setSavedData(data);
      console.log('AdminHomeEditor: Content saved successfully');
    } catch (err) {
      console.error('AdminHomeEditor: Error saving content:', err);
  ...
  try {
    ...
    console.error('💥 AdminHomeEditor crashed:', err);
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-gray-50 dark:bg-slate-900 text-red-600">
        <h1 className="text-3xl font-bold mb-4">Error loading Admin Home Editor</h1>
