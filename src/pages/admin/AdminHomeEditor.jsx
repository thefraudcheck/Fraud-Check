...
function AdminHomeEditor() {
  const defaultHeroData = {
    ...
    textColor: '#FFFFFF',
    height: 450,
    position: { y: 40 }, // Adjusted to show "FRAUD CHECK"
  ...
  useEffect(() => {
    try {
      const storedHomeData = JSON.parse(localStorage.getItem('homePageData')) || {};
      setData((prev) => ({
        ...prev,
        hero: {
          ...prev.hero,
          ...storedHomeData.hero,
          position: storedHomeData.hero?.position || { y: 40 },
          textAlignment: storedHomeData.hero?.textAlignment || 'bottom-left',
          height: storedHomeData.hero?.height || 450,
        },
        tipOfTheWeek: { ...prev.tipOfTheWeek, ...storedHomeData.tipOfTheWeek },
        keyFeatures: storedHomeData.keyFeatures || prev.keyFeatures,
      }));
      setSavedData((prev) => ({
        ...prev,
        hero: {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      setCarouselIndex((prev) => (prev + 1) % userReports.length);
    }, 4000);
  ...
  const handleSave = () => {
    ...
    try {
      ...
      setSavedData(data);
      // Force reload to ensure homepage reflects changes
      window.dispatchEvent(new Event('storage'));
    } catch (error) {
      console.error('Failed to save homePageData:', error);
    } finally {
  ...
  const nextSlide = () => {
    setCarouselIndex((prev) => (prev + 1) % Math.max(1, userReports.length));
  };
  const prevSlide = () => {
    setCarouselIndex((prev) =>
      (prev - 1 + Math.max(1, userReports.length)) % Math.max(1, userReports.length)
    );
  };
              ...
              >
                ...
                >
                  <div className="text-left max-w-xl">
                    ...
                    >
                      {data.hero.subtitle ||
                        'Use our tools to identify, report, and stay informed about fraud.'}
                    </p>
                    ...
                    {userReports.length === 0 ? (
                      <div className="flex-shrink-0 w-full p-4">
                        <div className="bg-white dark:bg-slate-800 rounded-xl shadow-md p-6 h-[420px] flex flex-col">
                          <p className="text-gray-500 dark:text-slate-400">No recent reports available.</p>
                        </div>
                      </div>
                    ) : (
                      userReports.map((report, idx) => (
                        <div key={idx} className="flex-shrink-0 w-full p-4" aria-label="View reported scam">
                          <div className="bg-white dark:bg-slate-800 rounded-xl shadow-md p-6 h-[420px] flex flex-col">
                            <h3 className="text-xl font-semibold mb-4 text-gray-900 dark:text-white">
                              {report.title}
                            </h3>
                            <p className="text-gray-600 dark:text-slate-300 mb-4">{report.description}</p>
                            <p className="text-gray-700 dark:text-slate-200 mb-4 flex-grow">
                              <span className="font-medium">Situation:</span> {report.situation}
                            </p>
                            <p className="text-red-600 font-medium mb-4">Red Flag: {report.redFlag}</p>
                            <div className="mt-auto">
                              <p className="text-sm text-gray-500 dark:text-slate-400 mb-4">
                                Reported: {report.date}
                              </p>
                              <button
                                className="inline-block px-4 py-2 bg-cyan-700 text-white rounded-lg transition-all"
                                disabled
                              >
                                View Full Report
                              </button>
