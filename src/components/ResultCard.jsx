...
function ResultCard({ isScam, message, riskLevel, redFlags = [], bestPractices = [], missedBestPractices = [] }) {
  const riskStyles = {
    ...
    'Moderate Risk': { bg: 'bg-yellow-100 dark:bg-yellow-900', text: 'text-yellow-700 dark:text-yellow-300', border: 'border-yellow-600' },
    'Low Risk': { bg: 'bg-green-100 dark:bg-green-900', text: 'text-green-700 dark:text-green-300', border: 'border-green-600' },
  };

  // Ensure data has fallback values and log for debugging
  const chartData = {
  ...
  return (
    <div className={`p-3 rounded-lg ${riskStyles[riskLevel]?.bg || (isScam ? 'bg-red-100 dark:bg-red-900' : 'bg-green-100 dark:bg-green-900')} border-l-4 ${riskStyles[riskLevel]?.border || 'border-gray-600'}`}>
